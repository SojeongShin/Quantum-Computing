...
function ret = make_quantum_circuit(nr_wire)
    ...
    function ret = swap__(wi_i, wi_j)
        ...
        %   |abcd> -> |bacd> -> |bcad> -> |bcda>
        wi_swap_indexes = wi_i:(wi_j-1);
        swp_opr = bubble(wi_swap_indexes);

        % bubble-down operation
        % e.g. continued
        %   |bcda> -> |bdca> -> |dbca>
        wi_swap_indexes = (wi_j-2):-1:wi_i;
        swp_opr = bubble(wi_swap_indexes) * swp_opr;
    ...
    function ret = tack_pin__(opr, wi_tgt)
        ...
        for i = 1:nr_pin
            ...
            if i-1 ~= qi2wi(qi_src(i)+1)  
                ...
                tacked_opr = swap__(i-1, qi2wi(qi_src(i)+1)) * tacked_opr;
        ...
        tacked_opr = (kron(opr, eye(2^(nr_wire-nr_pin)))) * tacked_opr;
        ...
        for i = 1:nr_wire
            if i-1 ~= qi2wi(i)          % if qubit i is not at wire i-1
                ...
                tacked_opr = swap__(i-1, qi2wi(i)) * tacked_opr;
   ...
   function ret = wire(opr, wi_indexes)
        opr = tack_pin__(opr, wi_indexes);    % tack pins of opr to wi_indexes
        qc_opr = opr * qc_opr; % wire opr to qc (update qc_opr)
        ret = qc_opr;
    end
    ...
    function ret = measure(wi_indexes)
        ...
        reg  = get_reg();               % current quantum state
        prnd = rand();                  % sample if psum >= prnd
        psum = 0;                       % check when psum >= prnd
        for ket_nr = 0:(2^nr_pin - 1)
            ket = ket_with_size__(ket_nr, nr_pin);
            prj = ket * ket';            % prj = |ket_nr><ket_nr|
            prj = tack_pin__(prj, wi_indexes);            % tack pins of prj to wi_indexes
            p = real(bra(reg) * prj * reg);              % p = P[ measure ket_nr ]               
            psum = psum + p;            % accumulate p to psum
            if psum >= prnd             % sample
                ret = ket_nr;        % measured value
                reg_collapsed = prj * reg;
                qc_opr = (reg_collapsed / norm(reg_collapsed)) * bra(ket__(0));     % collapse state
                return;
            end
        end
    end
